...
def plot_roc_pr_curves(y_test, y_proba, save_path=None, title_prefix="Logistic Regression"):
    """
    Generate ROC and Precision-Recall curves side by side, similar to the provided image
    """
...
def evaluate_at_multiple_thresholds(y_test, y_proba, include_optimal=True):
    """
    Comprehensive threshold evaluation focusing on practical usability
    Include optimal F1 threshold automatically
    """
    # Base thresholds
    ...
    if include_optimal:
        ...
        # Combine thresholds and remove duplicates
        all_thresholds = np.unique(np.append(base_thresholds, optimal_threshold))
        print(f"Optimal F1 threshold found: {optimal_threshold:.4f}")
    else:
        all_thresholds = base_thresholds
    ...
    for thresh in all_thresholds:
        ...
        is_optimal = False
        if include_optimal and abs(thresh - optimal_threshold) < 1e-6:
            is_optimal = True
        ...
        results.append({
            ...
            'is_optimal_f1': is_optimal
        })
    
    threshold_df = pd.DataFrame(results)
    
    # Store optimal threshold info for return
    if include_optimal:
        optimal_row = threshold_df[threshold_df['is_optimal_f1'] == True]
        if not optimal_row.empty:
            optimal_info = {
                'threshold': optimal_threshold,
                'f1_score': optimal_row['f1_score'].iloc[0],
                'precision': optimal_row['precision'].iloc[0],
                'recall': optimal_row['recall'].iloc[0]
            }
        else:
            optimal_info = {'threshold': optimal_threshold, 'f1_score': np.max(f1_scores)}
    else:
        optimal_info = None
    
    return threshold_df, optimal_info


def find_best_threshold_strategies(threshold_df, optimal_info=None):
    """
    Find thresholds optimized for different business scenarios
    Include optimal F1 threshold as a strategy
    """
    strategies = {}
    
    # Strategy 1: Best F1-score (use optimal_info if available)
    if optimal_info is not None:
        strategies['optimal_f1'] = {
            'threshold': optimal_info['threshold'],
            'metrics': optimal_info
        }
    elif not threshold_df.empty and threshold_df['f1_score'].max() > 0:
...
def plot_comprehensive_analysis(threshold_df, strategies, save_path=None):
    ...
    Mark optimal F1 threshold clearly
    """
    ...
    for strategy_name, strategy_info in strategies.items():
        ...
        f1 = strategy_info['metrics'].get('f1_score', 0)
        
        # Use different colors for optimal F1
        if strategy_name == 'optimal_f1':
            ax1.axvline(x=thresh, color='red', linestyle='-', linewidth=2, alpha=0.8)
            ax1.text(thresh, f1 + 0.02, 'OPTIMAL F1', rotation=90, fontsize=10, 
    ...
    if 'optimal_f1' in strategies:
        optimal_thresh = strategies['optimal_f1']['threshold']
        ax2.axvline(x=optimal_thresh, color='red', linestyle='-', linewidth=2, alpha=0.8)
    ...
    if 'optimal_f1' in strategies:
        optimal_thresh = strategies['optimal_f1']['threshold']
        ax3.axvline(x=optimal_thresh, color='red', linestyle='-', linewidth=2, alpha=0.8)
    ...
    for strategy_name, strategy_info in strategies.items():
        recall = strategy_info['metrics'].get('recall', 0)
        precision = strategy_info['metrics'].get('precision', 0)
        
        if strategy_name == 'optimal_f1':
            ax4.plot(recall, precision, 'ro', markersize=10, alpha=0.9, label='Optimal F1')
            ax4.text(recall + 0.01, precision + 0.01, 'OPTIMAL', fontsize=10, 
                    color='red', weight='bold')
        else:
            ax4.plot(recall, precision, 'mo', markersize=8, alpha=0.7)
...
def main(
    ...
    # 9. Evaluation with optimal threshold finding
    ...
    # Comprehensive threshold analysis WITH optimal threshold
    threshold_results, optimal_info = evaluate_at_multiple_thresholds(y_test, y_proba, include_optimal=True)
    ...
    if optimal_info:
        print(f"Optimal threshold: {optimal_info['threshold']:.4f}")
        print(f"Optimal F1-score: {optimal_info['f1_score']:.4f}")
        print(f"Precision at optimal: {optimal_info.get('precision', 'N/A'):.4f}")
        print(f"Recall at optimal: {optimal_info.get('recall', 'N/A'):.4f}")
    
    print("\n=== Threshold Analysis Results ===")
    print(threshold_results[['threshold', 'precision', 'recall', 'f1_score', 'tp', 'fp', 'is_optimal_f1']].round(3))
    
    # Find optimal strategies (now includes optimal F1)
    strategies = find_best_threshold_strategies(threshold_results, optimal_info)
    ...
    # 11. Visualizations (updated with optimal threshold marking)
    artifact_base_path = artifact_path.replace('.pkl', '')
    
# Generate ROC-PR curves (NEW - replaces individual plots)
    curves_plot_path = f"{artifact_base_path}_evaluation_curves.png"
    ...
    # Also save with the specific name you requested
    specific_plot_path = "logistic_pipeline_evaluation_curves.png"
    plot_roc_pr_curves(
        y_test, y_proba, 
        save_path=specific_plot_path,
        title_prefix="Logistic Regression"
    )
    ...
    # Comprehensive analysis plot (now shows optimal F1 clearly)
    ...
    # 2) Threshold sweep INCLUDING optimal threshold
    # Extract thresholds that were evaluated (includes optimal)
    evaluated_thresholds = threshold_results['threshold'].tolist()
    
    # Add specific thresholds we want to ensure are included
    required_thresholds = [0.3, 0.4, 0.5, 0.6, 0.7]
    if optimal_info:
        required_thresholds.append(optimal_info['threshold'])
    
    # Combine and deduplicate
    final_thresholds = sorted(list(set(evaluated_thresholds + required_thresholds)))
    ...
    # 3) Feature importance Top10
    try:
        pre = pipeline.named_steps["preproc"]
        feat_all = pre.get_feature_names_out()
        sel = pipeline.named_steps.get("feature_select", None)
    ...
    # 4) Error analysis using optimal threshold if available
    analysis_threshold = optimal_info['threshold'] if optimal_info else 0.5
    ...
    if optimal_info:
        ...
        print(f"Test ROC AUC: {curve_metrics['roc_auc']:.4f}")
        print(f"Test PR AUC: {curve_metrics['pr_auc']:.4f}")
        print(f"Baseline Precision: {curve_metrics['baseline_precision']:.4f}")
        print(f"Optimal F1 threshold: {optimal_info['threshold']:.4f}")
        print(f"Optimal F1 score: {optimal_info['f1_score']:.4f}")
...
if __name__ == '__main__':
    ...
    main(
        sampling_strategy='moderate_smote', 
        max_samples=200000
    )