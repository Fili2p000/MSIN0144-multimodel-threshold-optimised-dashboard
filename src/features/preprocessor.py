...
def get_feature_pipeline(
    ...
    if model_type == 'tree':
        ...
        # XGBoost：数值填中位数；有序编码；无序编码（XGBoost能处理分类特征）
        num_pipe = Pipeline([
            ('impute', SimpleImputer(strategy='median'))
        ])
        ord_pipe = Pipeline([
        ...
        # Logistic Regression: numerical features imputed then scaled; ordinal encoded then scaled; nominal one-hot encoded
        num_pipe = Pipeline([
            ('impute', SimpleImputer(strategy='median')),
        ...
        # CatBoost: numerical features only need imputation; categorical features keep original values for CatBoost internal handling
        num_pipe = Pipeline([
            ('impute', SimpleImputer(strategy='median'))
        ])
        # For CatBoost, categorical features only need missing value imputation, no encoding required
        ord_pipe = Pipeline([
            ('impute', SimpleImputer(strategy='constant', fill_value='Missing'))
        ])
        nom_pipe = Pipeline([
